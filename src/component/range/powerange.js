...
Horizontal.prototype.onmousedown = function (e) {
  ...
  // 增加
  if(hasClass(e.target,'handle-max')){
    this.handleOffsetX = this.handlemax.offsetLeft
    this.restrictHandleX = this.slider.offsetWidth - this.handlemax.offsetWidth
  }else{
...
Horizontal.prototype.generate = function () {
  var elems = {
    'handleMin': {
      'type': 'span',
      'selector': 'range-handle handle-min'
    },
    'handleMax': {
      'type': 'span',
      'selector': 'range-handle handle-max'
    },
    'min': {
      'type': 'span',
      'selector': 'range-min'
...
Horizontal.prototype.bindEvents = function () {
  ...
  this.handle = this.slider.querySelector('.handle-min')
  ...
  this.handlemax = this.slider.querySelector('.handle-max')
  // console.log(this.handle);
  this.touch2 = events(this.handlemax, this)
